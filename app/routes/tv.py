...
def tv_historico(id_veiculo: int, horas: int = 24):
    ...
    db  = get_db()
    cur = db.cursor()
    # Busca tecnicos vinculados ao veiculo (posse ativa ou recente)
    cur.execute("""
        SELECT DISTINCT p.id_tecnico
        FROM ht_veiculo_posse p
        WHERE p.id_veiculo = ?
          AND (p.entregue_em IS NULL
               OR p.entregue_em >= datetime('now','-3 hours','-7 days'))
    """, (id_veiculo,))
...
def veiculos_ativos():
    """Veiculos com posse ativa que tiveram GPS nos ultimos 7 dias."""
    db  = get_db()
    cur = db.cursor()
    # Usar ht_veiculo_posse (posse ativa) em vez de ht_tecnico_veiculo (diario)
    cur.execute("""
        ...
        LEFT JOIN ht_gps_track g ON g.id_tecnico = p.id_tecnico
            AND g.registrado_em >= datetime('now','-3 hours','-7 days')
